...
public class PTTF_AUTO_RED extends LinearOpMode implements ServoVariables{
    ...
    private void Right_Gyro(double degrees, double pwr, double sloommultiplier) throws InterruptedException{
        ...
        while (opModeIsActive() && (gyro.getHeading() < (degrees - 10) || gyro.getHeading() > 290)){
            LFdrive.setPower(pwr);
            LBdrive.setPower(pwr);
            RFdrive.setPower(-pwr);
            RBdrive.setPower(-pwr);
            idle();
            telemetry.addData("gyro loop 1", gyro.getHeading());
            telemetry.update();
        ...
        while (opModeIsActive() && loop){
            while (opModeIsActive() && !(gyro.getHeading() == degrees)){
                ...
                telemetry.addData("gyro loop 2", gyro.getHeading());
                telemetry.update();
            }
            LFdrive.setPower(0);
            LBdrive.setPower(0);
        ...
        while (opModeIsActive()&&loop) {
            if (Rcolor.red() > Rcolor.blue()) {
                ...
                if (Rcolor.red() < Rcolor.blue()) {
                    ...
                    telemetry.addData("red < blue", "");
                    telemetry.update();
                    Left_Sideways(42, 0.3);
                    Reverse(60, 0.3);
                    Forward(80, 0.5);
                    Left_Sideways(60, 0.2);
                    loop = false;
                } else {
    ...
    private void FollowWallRight(double omw, double pwr, double pwrmultiplier, double afstand, double threshold) throws InterruptedException{
        ...
        while (loop && opModeIsActive()){
            ...
            if (tmp > 82) FollowPowerTurn = 0.16;
            if (tmp < 82) FollowPowerTurn = -0.16;
            if (tmp == 82) FollowPowerTurn = 0;
        ...
        while (Flight.getRawLightDetected() < threshold && Blight.getRawLightDetected() < threshold && opModeIsActive()){
            ...
            LFdrive.setPower(pwr * pwrmultiplier + FollowPowerFront);
            LBdrive.setPower(-pwr * pwrmultiplier + FollowPowerFront);
            RFdrive.setPower(-pwr * pwrmultiplier + FollowPowerFront);
            RBdrive.setPower(pwr * pwrmultiplier + FollowPowerFront);
        }
    ...
    */
    ...
    @Override public void runOpMode() throws InterruptedException {
        ...
        Left_Gyro(317, 0.3, 0.48);
        Forward(488, 0.5);
        Right_Gyro(81, 0.3, 0.42);


        DriveToLineLeft(0.32, threshold);
        Reverse(20, 0.2);
        Push1();

        FollowWallRight(333, 0.5, 0.22,17, threshold);
        Push2();
